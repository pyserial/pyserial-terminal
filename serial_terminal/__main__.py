        ...
        try:
            while self.alive:
                ...
                if menu_active:
                    ...
                    if len(c) > 1:
                        c = self.escape_encoder.translate_named_key(c)
                    text = c
...
def main(default_port=None, default_baudrate=9600, default_rts=None, default_dtr=None, serial_instance=None):
    ...
    if not args.quiet:
        sys.stderr.write('---pySerial-terminal on {p.name}  {p.baudrate},{p.bytesize},{p.parity},{p.stopbits} ---\n'.format(
            p=miniterm.serial))
